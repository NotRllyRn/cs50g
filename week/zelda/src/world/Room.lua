...
function Room:update(dt)
    ...
    for i = #self.entities, 1, -1 do
        ...
        if entity.health <= 0 then
            -- add game object heart to the room by chance
            if not entity.dead and math.random(10) == 1 then
                ...
                -- pick random heart amount
                heart.health = math.random(2)
                heart.state = ({'full', 'half'})[heart.health]

                heart.consumable = true

                -- define a function for the heart that will add health to the player
                heart.onCollide = function()
                    self.player.health = math.min(6, self.player.health + heart.health)
                end
